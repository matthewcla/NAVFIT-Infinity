...
export function ReportAdminTab({ formData, setFormData, readOnly = false }: ReportAdminTabProps) {
    const [isHeaderCollapsed, setIsHeaderCollapsed] = useState(false);

    // Keep the "Member Identity" block somewhat collapsible if desired, 
    // or arguably in "Full Mode" we just show everything scrolled.
    // The original ReportEditor had a collapse toggle. We'll keep it.
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                    ...
                    <div className="md:col-span-4 space-y-1.5">
                        ...
                        <input
                            ...
                            value={formData.memberId} // Note: Original editor bound name to memberId? That seems odd if memberId is a UUID. 
                            // Checking ReportEditor.tsx: value={formData.memberId}. 
                            // Ideally this should be a name field, but we'll stick to the port.
                            // WAIT: memberId is usually the UUID. The name should be separate.
                            // In Report type, is there a 'name' field?
                            // formData.memberId suggests it might be displayed, BUT normally memberId is hidden ID.
                            // Let's check the type definition later, for now we port exactly as is.
