...
export function SailorProfiles() {
    ...
    const groupedRoster = useMemo(() => {
        ...
        // Assuming rank is string, sorting might need logic if not strictly alphabetic order of paygrade.
        // For now, let's sort by Paygrade if available, else Name.
    ...
    // Maybe custom sort order for buckets? URL first, etc.
    const orderedKeys = ['URL', 'RL', 'IWL', 'STAFF', 'LDO', 'CWO', 'RES LDO/CWO', 'Unknown'].filter(k => groupKeys.includes(k));
    // Add any remaining keys that weren't in the explicit list
    groupKeys.forEach(k => {
            ...
            <div className="w-96 border-r border-slate-200 bg-white flex flex-col h-full">
                <div className="p-4 border-b border-slate-200">
                    ...
                    <div className="relative">
                        <input
                            type="text"
                            placeholder="Search by name, rank, designator..."
                            value={searchQuery}
                                    ...
                                    <div className="bg-slate-50/50">
                                        {members.map(member => (
                                            <div
                                                ...
                                                className={`px-4 py-3 border-l-4 cursor-pointer hover:bg-white transition-all ${
                                        ...
                                        <div key={report.id} className="p-5 hover:bg-slate-50 transition-colors flex items-center justify-between group">
                                            <div className="flex items-center gap-4">
                                                <div className={`w-10 h-10 rounded-lg flex items-center justify-center border ${
                                                    report.type === 'Periodic' ? 'bg-blue-50 text-blue-600 border-blue-100' :
